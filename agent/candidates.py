...
watermark) so the delta engine decides NEW/ONGOING/RESOLVED week-over-week. Lifecycle
(eol/deprecation) entries are version-matched to the cycle in use to avoid cross-cycle noise."""
...
def _applies(ce: dict, version_in_use: str) -> bool:
    """Lifecycle (eol/deprecation) entries apply only to the cycle in use; others apply to the tech."""
    if ce.get("changeType") not in _LIFECYCLE:
        return True
    cycle = _cycle_of(ce)
    ...
    # Boundary-aware: cycle "8.0" matches "8.0"/"8.0.1" but NOT "8.2"; cycle "18" matches "18"/"18.0.0" but NOT "8".
    return normalized == cycle or normalized.startswith(cycle + ".")
