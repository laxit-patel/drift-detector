...
def _headers(accept):  # pragma: no cover - real GitHub HTTP
    # A scan issues ~64 requests (1 tree + ~63 files); the unauthenticated limit is 60/hr,
    # which would silently 403 the tail. Send GITHUB_TOKEN (5000/hr) when available.
    h = {"Accept": accept, "User-Agent": "change-monitor/1.0"}
...
def _default_fetch_tree():  # pragma: no cover - real GitHub HTTP
    ...
    r = requests.get(url, timeout=30, headers=_headers("application/vnd.github+json"))
    r.raise_for_status()
...
def _default_fetch_raw(path):  # pragma: no cover - real GitHub HTTP
    ...
    r = requests.get(url, timeout=30, headers=_headers("application/vnd.github.raw"))
    r.raise_for_status()
    return r.text
